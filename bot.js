...
client.on('interactionCreate', async interaction => {
  ...
  if(commandName === 'generate') {
    ...
    for(let i = 0; i < amount; i++) {
      ...
      KEYS[key] = { hwid: null, expires, createdAt: new Date().toISOString(), createdBy: interaction.user.tag }
      newKeys.push(key)
    }
    saveKeys(KEYS)
    ...
    const embed = new EmbedBuilder()
      ...
      .addFields(
        ...
        { name: 'Status', value: 'Unlocked (not yet used)', inline: true },
      )
    ...
    if(user) {
      try {
        const dmEmbed = new EmbedBuilder()
          ...
          .setDescription(keyList)
          .addFields({ name: 'Expires', value: expires || 'Never', inline: true })
  ...
  // ── /listkeys ────────────────────────────────────────
    ...
    const recent = Object.entries(KEYS).slice(-10).map(([k, v]) => {
      const status = v.hwid ? '🟢' : '🔓'
      return `${status} \`${k}\` — ${v.expires || 'no expiry'}`
    }).join('\n') || 'No keys yet'
